...
Ajoutez un moyen de paiement (l'usage est facturé à la consommation, quelques centimes pour des tests).
Allez dans API Keys → Create Key, copiez la clé (commence par `sk-ant-...`).
2. Déployer
Option A — sans rien installer (glisser-déposer)
Compressez ce dossier `deploy/` en `.zip`.
Sur vercel.com, créez un compte, puis Add New → Project → Deploy et déposez le dossier (ou importez-le).
